...
export class Root {
    ...
    _redraw() {
        ...
        try {
            // **** YOUR CODE HERE ****
            // clear the canvas
            this._canvasContext.clearRect(0, 0, this.owningCanvas.width, this.owningCanvas.height);
            // draw each child in turn (in reverse of drawn order)
            for (let chIndx = this.children.length - 1; chIndx >= 0; chIndx--) {
                const child = this.children[chIndx];
                // save the state of the drawing context
                this._canvasContext.save();
                // move to the child's position
                this._canvasContext.translate(child.x, child.y);
                // draw the child
                child.draw(this._canvasContext, this.doDebugOutput);
                // restore the state of the drawing context
